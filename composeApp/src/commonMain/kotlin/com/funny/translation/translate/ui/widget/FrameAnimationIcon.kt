...
data class FrameAnimationIconState(
    val frameIconNames: List<String> = listOf(),
    ...
    fun next(){
        currentFrameIdx++
        if(currentFrameIdx == frameIconNames.size) currentFrameIdx = 0
    }
    fun play(){
        isPlaying = true
    }
...
fun rememberFrameAnimIconState(
    frameIconNames: List<String> = listOf(),
    initialPlaying: Boolean = false,
    duration: Long = 500
) = rememberSaveable(saver = listSaver(
    save = { listOf(it.isPlaying)},
    restore = { FrameAnimationIconState(frameIconNames, it[0], duration) }
)) {
    FrameAnimationIconState(frameIconNames, initialPlaying, duration)
}
...
//        Log.d(TAG, "FrameAnimationIcon: Launched $state")
        while (state.isPlaying && state.frameIconNames.isNotEmpty()){
            delay(state.duration / state.frameIconNames.size)
            state.next()
//            Log.d(TAG, "FrameAnimationIcon: 当前帧数：${state.currentFrameIdx}")
        }
    }

    FixedSizeIcon(painter = painterDrawableRes(state.frameIconNames[currentFrame]), contentDescription = contentDescription, tint = tint, modifier = modifier)
}