...
internal fun CurrentCorpusList(
    ...
    CorpusList(
        ...
        removeTerm = {
            corpus.remove(it)
            askForOther.ask(
                message = ResStrings.message_remove_from_all_corpus,
                action = {
                    allCorpus.remove(it)
                }
            )
        ...
        modifyTerm = { origin, target ->
            corpus.modify(origin, target, alert = true)
            askForOther.ask(
                message = ResStrings.message_modify_all_corpus,
                action = {
    ...
    fun ask(message: String, action: SimpleAction) {
        coroutineScope.launch {
            snackHostState.showSnackbar(
                message = message,
                actionLabel = ResStrings.message_confirm,
                onClick = action
            )
        }
    }
                ...
                trailingContent = {
                    IconButton(onClick = { removeTerm(term) }) {
                        FixedSizeIcon(
                            ...
                            contentDescription = ResStrings.delete,
            ...
            text = {
                ...
                Column {
                    ...
                    TextField(
                        ...
                        label = {
                            Text(text = ResStrings.target_text)
                        },
